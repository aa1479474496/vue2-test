<template name="abc">
    <div>
        ...
        <!--<info></info>-->
        <ul>
            <li v-for="(item,index) in list" v-bind:class="[liIndex == index ? 'active' : '']" :key="item.id" @click="changeColor(index)">
                {{item}}--{{index}}
            </li>
        </ul>
        <div id="abc">123</div>
        <div>{{testHan()}}</div>
        <div>{{test2}}</div>
        <button type="button" @click="testIndex++">testIndex加法运算</button>
    </div>
</template>
<style>
li {
